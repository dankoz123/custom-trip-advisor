...
For a single-page visual of the full pipeline — from `City, Country` input through grounded attractions to the final itinerary — see the editable deck:

**[process-flow-singlePage.pptx](assets/process-flow-singlePage.pptx)**

The slide shows all 7 stages on one canvas, with the external tools and services used at each step. Editable in PowerPoint, Keynote, or Google Slides.
